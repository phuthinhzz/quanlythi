...
const ImportStudents = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/v1/admin/classes/import-students/${id}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
  ...
  const downloadTemplate = () => {
    const fileId = "1WL-HExC2TExKb8GUP092g6ElPR7Ju3Wo"; // Lấy ID từ URL Google Drive
    const downloadUrl = `https://drive.google.com/uc?export=download&id=${fileId}`;

    const link = document.createElement("a");
    link.href = downloadUrl;
    link.download = "Template_AddUsserToClass.xlsx";
    link.click();
  };
          ...
          <div className="px-8 pt-8 pb-10">
            ...
            <div className="mb-8">
              <button
                ...
                className="px-6 py-2.5 flex items-center gap-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition duration-200 font-medium"
              >
                <svg
                  className="w-5 h-5"
                  fill="none"
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg px-6 py-10 bg-gray-50 hover:bg-gray-100 transition duration-200">
                <div className="flex flex-col items-center justify-center space-y-4">
                  ...
                  <p className="text-sm text-gray-500 text-center">
                    {file
                      ? file.name
                      : "Import template should include: MSSV, Name, Email"}
                  </p>
                </div>
              </div>
                    ...
                    <div className="flex">
                      ...
                      <div className="ml-3">
                        <p className="text-sm text-green-700">
                          Successfully imported {result.success.length} students
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  {result.errors.length > 0 && (
                    <div className="bg-orange-50 border-l-4 border-orange-500 p-4 rounded-lg">
                      <div className="mb-2 text-sm text-orange-700 font-medium">
                        Failed to import {result.errors.length} students:
                      </div>
